...
import { apiService, NFT, Transaction, IncentiveBalance } from '../services/api';
...
const WalletTab: React.FC = () => {
  ...
  const loadUserData = async () => {
    try {
      ...
      try {
        const balance = await apiService.getIncentiveBalance(accountId as string);
  ...
  const getNFTIcon = (type: string) => {
    return type === 'consent' ? <CheckIcon color="success" /> : <NFTIcon color="primary" />;
  };
    ...
    <Box>
      ...
      <Card sx={{ p: 2, mb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box>
            <Typography variant="h6" sx={{ mb: 1 }}>Hedera Account</Typography>
            <Typography variant="body2" color="text.secondary">
              {accountId}
            </Typography>
          </Box>
      ...
      {/* GDI Token Balance */}
      <Card sx={{ p: 2, mb: 2 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>GDI Token Balance</Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      {/* My NFTs */}
      <Card sx={{ p: 2, mb: 2 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>My NFTs ({nfts.length})</Typography>
        {loading ? (
